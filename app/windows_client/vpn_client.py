"""
VPN Client - Windows proqramı.
PyQt5 əsaslı GUI, login, connect/disconnect, system tray.
"""
import sys
import os
import json
...
class APIClient:
    ...
    def login(self, username, password):
        ...
        try:
            ...
            return {"success": False, "message": "Serverə qoşulmaq mümkün olmadı"}
    ...
    def connect(self):
        ...
        try:
            ...
            return {"success": False, "message": "Server əlçatan deyil"}
        except Exception as e:
            return {"success": False, "message": str(e)}
    ...
    def disconnect(self):
        ...
        try:
            ...
            self.token = None
            return {"success": True, "message": "Bağlantı kəsildi"}
...
class ProxyThread(QThread):
    """Windows-da system proxy-ni tənzimləyir."""
    ...
    def run(self):
        ...
        try:
            ...
            self.error_signal.emit(f"Proxy xətası: {e}")
    ...
    def set_system_proxy(self, enable):
        """Windows registry vasitəsilə system proxy tənzimləmə."""
        ...
        try:
            ...
            raise Exception(f"Registry xətası: {e}")
...
class LoginWindow(QWidget):
    ...
    def init_ui(self):
        ...
        subtitle = QLabel("Təhlükəsiz bağlantı")
        ...
        self.username_input.setPlaceholderText("İstifadəçi adı")
    ...
    def check_server(self):
        """Server əlçatanlığını yoxla."""
        if self.api.ping():
            self.status_label.setText("Server: Əlçatan")
            self.status_label.setStyleSheet("color: #4caf50;")
        else:
            self.status_label.setText("Server: Əlçatmaz")
    ...
    def do_login(self):
        ...
        if not username or not password:
            self.error_label.setText("İstifadəçi adı və parol daxil edin")
            return

        self.login_btn.setEnabled(False)
        self.login_btn.setText("Gözləyin...")
        ...
        if result.get("success"):
            ...
            self.error_label.setText(result.get("message", "Xəta baş verdi"))


class MainWindow(QWidget):
    """Əsas VPN pəncərəsi - connect/disconnect."""
    ...
    def init_ui(self):
        ...
        # Başlıq
        header = QHBoxLayout()
        user_label = QLabel(f"Salam, {self.user_data.get('user', {}).get('username', '')}")
        ...
        # Status
        self.status_label = QLabel("Bağlantı Kəsik")
        ...
        expire_label = QLabel(f"Müddət: {expire or 'Limitsiz'}")
        expire_label.setObjectName("infoLabel")
        info_layout.addWidget(expire_label)

        self.connection_time_label = QLabel("Bağlantı müddəti: --:--:--")
    ...
    def init_tray(self):
        ...
        self.tray_connect_action = QAction("Qoşul", self)
    ...
    def do_connect(self):
        ...
        self.status_label.setText("Qoşulur...")
        ...
        if result.get("success"):
            ...
            self.status_label.setText("Qoşulub")
            self.status_label.setStyleSheet("color: #4caf50;")
            self.connect_btn.setText("Ayrıl")
            self.connect_btn.setObjectName("disconnectBtn")
            self.connect_btn.setStyle(self.connect_btn.style())
            self.tray_connect_action.setText("Ayrıl")
            self.tray_icon.setToolTip(f"{APP_NAME} - Qoşulub")
        else:
            self.status_label.setText("Xəta: " + result.get("message", ""))
            self.status_label.setStyleSheet("color: #e94560;")
    ...
    def do_disconnect(self):
        ...
        self.status_label.setText("Bağlantı Kəsik")
        self.status_label.setStyleSheet("color: #e94560;")
        self.connect_btn.setText("Qoşul")
        self.connect_btn.setObjectName("connectBtn")
        self.connect_btn.setStyle(self.connect_btn.style())
        self.tray_connect_action.setText("Qoşul")
        self.tray_icon.setToolTip(f"{APP_NAME} - Ayrılıb")
        self.connection_time_label.setText("Bağlantı müddəti: --:--:--")
    ...
    def update_status(self):
        ...
        if result.get("success"):
            ...
            self.traffic_label.setText(f"Trafik: {traffic:.1f} MB istifadə olunub")

    def update_time(self):
        """Bağlantı müddətini yenilə."""
        if self.connect_time:
            ...
            self.connection_time_label.setText(
                f"Bağlantı müddəti: {hours:02d}:{minutes:02d}:{seconds:02d}"
            )

    def closeEvent(self, event):
        """Pəncərə bağlananda tray-ə minimize et."""
        event.ignore()
        self.hide()
        self.tray_icon.showMessage(
            APP_NAME,
            "Proqram arxa planda işləyir",
